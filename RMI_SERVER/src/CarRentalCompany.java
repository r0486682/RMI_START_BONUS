...
public class CarRentalCompany implements RemoteInterface {
    ...
    @Override
    public List<Reservation> getAllReservationsForClient(String clientName) throws RemoteException {
        List<Reservation> reservations = new ArrayList<Reservation>();
        for (Car car : cars)
            reservations.addAll(car.getAllReservationsForClient(clientName));
        return reservations;
    }

    @Override
    public int getReservationsCountForCarType(String carType) throws RemoteException {
        int count = 0;
        for (Car car : cars) {
            if (car.isCarType(carType))
                count += car.getReservationsCount();
        }
        return count;
    }
